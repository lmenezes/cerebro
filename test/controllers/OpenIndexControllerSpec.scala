...
object OpenIndexControllerSpec extends Specification with Mockito {

  def is =
    s2"""
    OpenIndexController should                           ${step(play.api.Play.start(FakeApplication()))}
      invoke openIndex                                  $openIndex
      should throw exception if indices param is missing $missingIndices
                                                         ${step(play.api.Play.stop(FakeApplication()))}
      """

  val controller = new OpenIndexController
  ...
  def openIndex = {
    ...
    client.openIndex("a,b,c", "somehost") returns Future.successful(ElasticResponse(200, expectedResponse))
