...
object RefreshIndexControllerSpec extends Specification with Mockito {

  def is =
    s2"""
    RefreshIndexController should                        ${step(play.api.Play.start(FakeApplication()))}
      invoke refreshIndex                                $refreshIndex
      should throw exception if indices param is missing $missingIndices
                                                         ${step(play.api.Play.stop(FakeApplication()))}
      """

  val controller = new RefreshIndexController
  ...
  def refreshIndex = {
    ...
    client.refreshIndex("a,b,c", "somehost") returns Future.successful(ElasticResponse(200, expectedResponse))
    val response = Await.result(controller.processElasticRequest(CerebroRequest(body), client), Duration("1s"))
    there was one(client).refreshIndex("a,b,c", "somehost")
